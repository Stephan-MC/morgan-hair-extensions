...
export class CookieService {
	...
	private readonly isServer = isPlatformServer(this._platformId);

	constructor() {
		// --- Client-Side Hydration ---
		// When the app runs in the browser, this logic takes over.
		if (!this.isServer) {
			// update browser cookies if and only if required cookies are not found
			if (!this._document.cookie.match(/\bXSRF-TOKEN\b/)) {
				// 1. Get cookie strings transferred from the server.
				const serverCookieStrings = this._transferState.get(
					COOKIE_STATE_KEY,
					[],
				);
	...
	/**
	 * Gets a read-only map of all cookies.
	 * @returns A new Map instance containing all cookies.
	 */
	getAll(): Map<string, string> {
		// Return a new map to prevent external modification of the internal signal's value.
